...
public class HomeController {
	...
	// @RequestMapping(value = "getData/{viewNm}") get일경우 -> /eis/getData/google ->
	// json타입으로 나옴
	...
	public void getImg(HttpServletRequest req, HttpServletResponse res) {
		...
		// base64 이미지로 저장
		try {
			String path = "D:/jiianiii/IDE/workspace/Dashboard/src/main/webapp/resources/img/";
			...
			out.write(byteDat);
			result.put("status", 1);
			result.put("url", "/eis/resources/img/" + fileNm);
		} catch (Exception e) {
			e.printStackTrace();
			result.put("status", 0);
		} finally {
	...
	public void getImgList(HttpServletRequest req) {
		...
		String imgData = req.getParameter("imgData");
		List<String> imgList = JSONArray.fromObject(imgData);

		for (int i = 0; i < imgList.size(); i++) {
			String data = imgList.get(i).toString();
			OutputStream out = null;
			// base64 이미지로 저장
			try {
				...
				File file = new File(path + "img" + i + ".png");
				out = new FileOutputStream(file);
				byte[] byteDat = Base64Utils.decodeFromString(data.replace("data:image/png;base64,", ""));
				out.write(byteDat);
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
